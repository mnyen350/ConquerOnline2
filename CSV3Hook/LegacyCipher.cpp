...
void CLegacyCipher::Encrypt(uint8_t* pBuf, int32_t Length)
{
	...
	for (int32_t i = 0; i < Length; i++)
	{
		if (BufIV != NULL)
		{
			pBuf[i] ^= (uint8_t)(BufIV[(uint8_t)(EncryptCounter & 0xFF) + 0]);
			pBuf[i] ^= (uint8_t)(BufIV[(uint8_t)(EncryptCounter >> 8) + K]);
		}
		pBuf[i] = (uint8_t)(pBuf[i] >> 4 | pBuf[i] << 4);
		pBuf[i] ^= (uint8_t)0xAB;
		EncryptCounter++;
	}
}
void CLegacyCipher::Decrypt(uint8_t* pBuf, int32_t Length)
{
	...
	for (int32_t i = 0; i < Length; i++)
	{
		...
		if (BufIV != NULL)
		{
			pBuf[i] ^= (uint8_t)(BufIV[(uint8_t)(DecryptCounter & 0xFF) + 0]);
			pBuf[i] ^= (uint8_t)(BufIV[(uint8_t)(DecryptCounter >> 8) + K]);
		}
		DecryptCounter++;
	}
}
